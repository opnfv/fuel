...
export CENTOS_VERSION_ID=centos-7.2.1511
...
CENTOS_MIRROR_URL=$(shell curl -sSf http://$(MIRROR_MOS_UBUNTU)/pkgs/$(CENTOS_VERSION_ID)-latest.htm)
CENTOS_MIRROR_ID=$(shell expr ${CENTOS_MIRROR_URL} : '.*/\(centos-.*\)')
MIRROR_CENTOS_ROOT=pkgs/snapshots/${CENTOS_MIRROR_ID}
export MIRROR_CENTOS=http://${MIRROR_MOS_UBUNTU}/${MIRROR_CENTOS_ROOT}
LATEST_TARGET_MOS_CENTOS=$(shell curl -sSf http://${MIRROR_MOS_UBUNTU}//mos-repos/centos/mos-master-centos7//os.target.txt | sed '1p; d')
export MIRROR_FUEL=http://${MIRROR_MOS_UBUNTU}//mos-repos/centos/mos-master-centos7//${LATEST_TARGET_MOS_CENTOS}/x86_64

# Ubuntu overrides
UBUNTU_MIRROR_URL=$(shell curl -sSf http://${MIRROR_MOS_UBUNTU}/pkgs/ubuntu-latest.htm)
UBUNTU_MIRROR_ID=$(shell expr $UBUNTU_MIRROR_URL : '.*/\(ubuntu-.*\)')
LATEST_TARGET_MOS_UBUNTU=$(shell curl -sSf http://${MIRROR_MOS_UBUNTU}//mos-repos/xenial//master.target.txt | sed '1p; d')
export MIRROR_MOS_UBUNTU_ROOT=/mos-repos/xenial//${LATEST_TARGET_MOS_UBUNTU}
