...
**NOTE**: The deployment needs OPNFV Pharos project as input for hardware and network
(PDF and IDF files), which is retrieved from upstream during deploy. When installing
a new POD, you can pass -b flag to the deploy script with the path for local folder
containing you PDF and IDF. Make sure you keep the same folder structure as the Pharos project.

   .. code-block:: bash

       $ ci/deploy.sh -b file://<path_to_your_IDF/PDF> \
                      -l <lab_name> \
                      -p <pod_name> \
                      -s <scenario> \
                      -D \
   ...
   - <path_to_your_IDF/PDF> is the absolute path to your input files. It needs to have the same
     folder structure as Pharos: <absolute_path>/labs/<lab_name>
   - <lab_name> is the same as the folder in the path above
