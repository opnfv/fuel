...
also be pre-configured (e.g. admin_br, mgmt_br, public_br).
   ...
   - The management bridge (mgmt_br) is required for testing suits(functest/yardstick), it is suggested to preconfig it for debugging purpose.
   - The public bridge (public_br) is also nice to have for debugging purpose, but not mandatory.

The user running deploy script on the Jumpserver should belong to "sudo" and "libvirt" groups, and have passwordless sudo access.

The following example add the groups to the user "jenkins"
...
While not mandatory, upgrading the kernel and QEMU on the Jumpserver is also highly recommended (especially on AArch64 Jumpservers)."
...
In the baremetal deploy all bridges but "mcpcontrol" are Linux bridges. For the Jumpserver, it is required
to pre-config at least the admin_br bridge for the PXE/Admin.
