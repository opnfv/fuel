...
For DPDK enabled deployments the following additional parameters can be defined into the IDF files under
group "reclass". Those value will overwrite the default configuration values in
the Fuel repository. Since huge pages and cpu pinning can also apply to non-DPDK scenario, they can be
defined in "common", respectivly "dpdk".
