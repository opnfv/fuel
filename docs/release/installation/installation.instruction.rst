...
|                            | hosts a Salt Master VM and each of the VM nodes of     |
|                            | the control plane in the virtual deploy                |
+----------------------------+--------------------------------------------------------+
...
**NOTE:** All nodes and the Jumpserver must have the same architecture (either x86_64, either aarch64)
...
The installation is done automatically with the deploy script, which will:
   ...
   - Create a MaaS Node VM on the Jumpserver which will provision the targets (only for baremetal)
...
For virtual deploys all the targets are VMs on the Jumpserver. If a real bridge exists for a given
