...
On the systems already deployed, if the trusted key list shows KEYEXPIRED for Armband repository,
try to manually refresh the keyring to get the updated one.

.. code-block:: bash

    //refresh the keyring on the Jumpserver
    $ sudo apt-key adv --keyserver keys.gnupg.net --recv 798AB1D1

    //refresh the keyring on all nodes that have already been deployed, login to cfg01 and do:
...
**Note**: The Jumpserver still needs to specify the fixed admin ipaddress for legacy reasons.
...
The "fuel" section defines several parameter groups required by the fuel installer:
...
The following parameters can be defined into the IDF files under group "reclass". Those value will
overwrite the default configuration values in Fuel repository.

- nova_cpu_pinning: List of CPU cores to be tuned.
- compute_hugepages_size: Size of persistent huge pages. Valid values are '2M' and '1G'.
- compute_hugepages_count: Number of persistent huge pages.
...
The full description of the PDF and IDF file structure are avaiable as yaml schemas at the following
location in Pharos repository. Input files provided to the installer will be validated against the schema.

- *pharos/config/pdf/pod1.schema.yaml*
- *pharos/config/pdf/idf-pod1.schema.yaml*
