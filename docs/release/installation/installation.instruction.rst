...
also be pre-configured (e.g. admin_br, mgmt_br, public_br).
   ...
   - The management bridge (mgmt_br) is required for testing suits(e.g functest/yardstick), it is
     suggested to preconfig it for debugging purpose.
   - The public bridge (public_br) is also nice to have for debugging purpose, but not mandatory.

The user running deploy script on the Jumpserver should belong to "sudo" and "libvirt" groups,
...
**NOTE**: The install script expects 'libvirt' to be already running on the Jumpserver.
In case libvirt packages are missing, the script will install them;
but depending on the OS distribution, the user might have to start the 'libvirtd' service manually,
then run the deploy script again. Therefore, it is recommened to install libvirt-bin explicitly on
the Jumpserver before the deployment.
...
required to pre-config at least the admin_br bridge for the PXE/Admin.
...
#. Start the deploy script
   ...
   .. code-block:: bash
       ...
       $ ci/deploy.sh -l <lab_name> \
                      ...
                      -D <enalbe debug logging> \
                      -S <Storage dirtory for disk images>
