...
Logging in to cluster nodes is also possible from Salt master, where cluster hostnames can be used
to connect
...
services are installed in VMs on the baremetal targets (kvm servers). User *ubuntu* has sudo rights.

On aarch64 the VCP VMs can also be accessed with virsh console:
user *opnfv*, password *opnfv_secret*. This method does not apply to infrastructure
VMs (Salt master and MaaS). The example below is a connection to a controller VM.
The connection is made from the baremetal server kvm01, on an odl scenario.

    .. code-block:: bash

        $ ssh -o StrictHostKeyChecking=no -i  /var/lib/opnfv/mcp.rsa  -l ubuntu <kvm01_IP>
        ubuntu@kvm01:~$ virsh console ctl01.mcp-pike-odl-ha.local
