...
# Function check whether crudini is available, if not, installs
function chkCrudini () {
...
# Function check whether a python egg is available, if not, installs
function chkPPkg () {
    PKG="$1"
...
function envSetup () {
    apt-get install git python-pip python-all debhelper
    chkPPkg stdeb
    chkCrudini
}
...
EOFTD
    cat > /etc/init/tacker-server.conf <<EOFSC
...
# Funcion copies and installs python-tackerclient on all remaining cluster nodes
...
service tacker-server start
puppet apply --debug --verbose configure_tacker.pp
initctl reload tacker-server
}
