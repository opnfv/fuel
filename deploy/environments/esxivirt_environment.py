...
class EsxivirtEnvironment(ExecutionEnvironment):

    def __init__(self, storage_dir, dha_file, dea, root_dir):
        super(EsxivirtEnvironment, self).__init__(
            storage_dir, dha_file, root_dir)
	print("INIT ESXIVIRTENVIRONMENT")
        self.dea = dea
    ...
    def create_vms(self):
        ...
        for node_id in self.node_ids:
            vm_name = self.dha.get_node_property(node_id, 'esxivirtName')
            vm_template = '%s/%s' % (self.root_dir,
                                     self.dha.get_node_property(
                                         node_id, 'esxivirtTemplate'))
