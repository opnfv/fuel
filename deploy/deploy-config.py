...
def parse_arguments():
    ...
    parser.add_argument('-scenario-base-uri',
                        ...
                        action='store',
                        default=False,
                        help='Deploymen scenario base directory URI',
                        required=True)
    parser.add_argument('-scenario', dest='scenario', action='store',
                        default=False,
                        help='Deploymen scenario short-name (priority), or base file name (in the absense of a shortname defenition)',
                        required=True)
...
with open(kwargs["output_path"] + '/dea.yaml', "w") as f:
    f.write("\n".join(["""title: DEA.yaml file automatically generated from the configuration files stated in the "configuration-files" fragment below""",
            "version: " + str(calendar.timegm(time.gmtime())),
            "created: " + str(time.strftime("%d/%m/%Y")) + " " + str(time.strftime("%H:%M:%S")),
            "comment: none\n"]))
...
with open(kwargs["output_path"] + '/dha.yaml', "w") as f:
    f.write("\n".join(["""title: DHA.yaml file automatically generated from the configuration files stated in the "configuration-files" fragment below""",
                       "version: " + str(calendar.timegm(time.gmtime())),
                       "created: " + str(time.strftime("%d/%m/%Y")) + " " + str(time.strftime("%H:%M:%S")),
                       "comment: none\n"]))
