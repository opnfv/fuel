...
def parse_arguments():
    ...
    kwargs = {'dha_uri': args.dha_uri,
              ...
              'dea_pod_override_uri': args.dea_override_uri,
...
for module in deploy_scenario_conf["stack-extensions"]:
    print 'Loading configuration for module: ' + module["module"]
    response = urllib2.urlopen(kwarg["plugins_uri"] + '/' + module["module-config-name"] + '_' + module["module-config-version"] + '.yaml')
    final_dea_conf['settings']['editable'].update(yaml.load(response.read()))

# TBD Remove original dea-base and dea-override information and auto generate
...
response = urllib2.urlopen(kwargs["dha_base_uri"])
dha_base_conf = yaml.load(response.read())
dha_scenario_override_conf = deploy_scenario_conf["dha-override-config"]
final_dea_conf = dict(mergedicts(dha_base_conf, dha_scenario_override_conf))
