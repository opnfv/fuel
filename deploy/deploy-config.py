...
if dea_pod_override_conf:
    ...
    print 'Merging dea-base and dea-pod-override configuratin ....'
...
print 'Parsing deployment-scenario from: ' + kwargs["scenario_uri"] + "...."
...
if deploy_scenario_conf:
    ...
    deploy_scenario_sha = sha_uri(kwargs["scenario_uri"])
...
for module in deploy_scenario_conf["stack-extensions"]:
    ...
    dea_scenario_module_override_conf = module['module-config-override']
    final_dea_conf['settings']['editable'][module["module"]].update(dea_scenario_module_override_conf)
...
with open(kwargs["output_path"] + '/dea.yaml', "w") as f:
    ...
    f.write("    uri: " +  kwargs["scenario_uri"] + "\n")
