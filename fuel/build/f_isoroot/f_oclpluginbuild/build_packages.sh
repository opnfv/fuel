...
fetch_repo_sha() {
    pushd $SANDBOX_DIR
    REPO_PATH_LIST=$(xmllint --xpath '//manifest/project/@path' .repo/manifest.xml | sed -r 's/path=\"([^\"]+)\"/\1/g' | sed 's/tools\/packages//')
    for repo_path in $REPO_PATH_LIST
    do
        cd ${SANDBOX_DIR}/${repo_path} && git rev-parse HEAD >> ${SCRIPT_PATH}/${CACHE_ID_FILE}
    done
    popd
}
#
...
install_deps() {
    ...
    if [ "${TARGET_KERNEL}" == "$(uname -r)" ]; then
        ...
        echo "Cross build not yet implemented, exiting....."
        # TODO: Fix the ability to build with other kernel headers
        exit 1
    fi
}
#
