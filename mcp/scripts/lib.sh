...
function __kernel_modules {
  ...
  if [ "${__uname_m}" = 'x86_64' ]; then
    ...
    __srpm="kernel-${__uname_m}-${__uname_r%.${__uname_m}}.src.rpm"
  fi
  ...
  for ver in ${__versions}; do
    for comp in os updates; do
      ...
      if wget "${url}" -O "${image_dir}/${__srpm}" 2>&1 > /dev/null ; then
        __found='y'; break 2
      fi
    done
  done
  ...
  if [ "${__found}" = 'n' ]; then
    echo "[ERROR] Can't find the linux kernel SRPM for: ${uname_r}"
  ...
  (
    ...
    sudo cp drivers/block/nbd.ko /lib/modules/$(uname -r)/extra/
  )
