...
GH_SF="https://github.com/salt-formulas https://github.com/saltstack-formulas"
...
# create new local branch to match expected revision after filter-branch
git branch -f "${FORMULA_REV}"
for formula_dir in "${MCP_REPO_ROOT_PATH}/mcp/salt-formulas"/*; do
  ...
  if [ -e "${formula_dir}/.git" ]; then
    ...
    git clone "${MCP_REPO_ROOT_PATH}" "${formula_tmp}" -b "${FORMULA_REV}"
