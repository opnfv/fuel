...
-f Deploy on existing Fuel master
-F Do only create a Fuel master
-H Do not run fuel built in healt-check after successfull deployment
...
-s Deployment-scenario, this points to a deployment/test scenario file as
   ...
   scenario path.
-S Storage dir for VM images, defaut is fuel/deploy/images
...
if [ $DRY_RUN -eq 0 ]; then
    ...
    python deploy.py -s $STORAGE_DIR -b $PXE_BRIDGE $USE_EXISTING_FUEL $FUEL_CREATION_ONLY $NO_HEALTH_CHECK -dea ${SCRIPT_PATH}/config/dea.yaml -dha ${SCRIPT_PATH}/config/dha.yaml -iso $ISO
fi
popd > /dev/null
