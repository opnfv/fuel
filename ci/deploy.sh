...
-m Use single socket compute nodes. Instead of using default topology
   for virtual compute nodes created via libvirt, configure a single guest
   CPU socket and assign less RAM memory to keep the overall resource
   footprint lower.
