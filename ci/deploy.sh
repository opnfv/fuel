...
$(notify "OPTIONS:" 2)
  ...
  -m  Disable NUMA for compute nodes (only affects virtual computes)
...
-m Disable NUMA: Instead of using default topology for virtual compute
   nodes created via libvirt, configure a single guest CPU socket and
   assign less RAM memory.
...
export MCP_REPO_ROOT_PATH MCP_VCP MCP_DPDK_MODE MCP_STORAGE_DIR \
       MCP_VCP_NUMA MCP_JUMP_ARCH=$(uname -i)
